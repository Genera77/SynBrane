...
- Synth Parameters: master volume and the global arpeggiator (enable/pattern/rate) lead the panel, followed by mode (harmony/rhythm), tempo (30–300 BPM), rhythm multiplier (~0.1–1.0), waveform, attack/decay/sustain/release, detune, cutoff/resonance, loop length selector, loop playback/render buttons, and patch Save/Load.
- Patch JSON shape (v1):
```
{
  "version": 1,
