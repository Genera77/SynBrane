...
- Optional environment variables remain supported for overrides:
  ...
  - `SCALES_DIR` (default `<repo>/scales`, normalized to avoid duplicate trailing path segments like `CIP/CIP`)
  - `RENDER_OUTPUT_DIR` (default `<repo>/renders`, normalized to avoid duplicate trailing path segments like `CIP/CIP`)
