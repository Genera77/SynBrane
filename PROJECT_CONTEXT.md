...
- **Audio engines** (`server/audio/`)
  ...
  - `engine.js` — Node DSP fallback synthesizing harmony voices with selectable waveforms, ADSR envelopes, optional resonant low-pass filter, and arpeggiated rendering based on per-event pattern/rate fields. Rhythm voices layer a four-on-the-floor kick plus snares on beats 2 and 4, and map chord notes onto drum voices. Supports single-chord renders and multi-event sequences.
